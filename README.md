...
- `power_collective_lead_*`: uses the physical collective as an upward-only preview of a slow power-proxy increase. The default zero deadband preserves small deliberate collective changes; increase it only enough to suppress visible rudder jitter from a noisy collective sensor.
